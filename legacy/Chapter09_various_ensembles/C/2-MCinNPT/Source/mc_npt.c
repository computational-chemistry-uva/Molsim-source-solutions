  ...
  for(m=0;m<NumberOfCycles;m++)
  {
    for(n=0;n<CycleMultiplication;n++)
    {
      ...
      if(Ipart>=NumberOfParticles)
      {
        ...
        // MODIFICATION
        // Random Walk in V instead of ln(V):

//        Vnew=Vold+(RandomNumber()-0.5)*MaximumVolumeChange;

        // Note that another change has to be made in the acceptance
        // ratio a bit below.
        ...
        {
        // MODIFICATION
        // For a change in V instead of ln(V), replace the above line by:
/*
        if((!overlap)&&(RandomNumber()<exp(-Beta*Pressure*(Vnew-Vold)+log(Vnew/Vold)*((double)NumberOfParticles))))
*/
