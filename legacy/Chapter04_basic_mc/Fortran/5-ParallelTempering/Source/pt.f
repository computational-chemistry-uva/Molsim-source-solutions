...
C     (Instead of using Lready, students can also use a Goto command,
C     even though it is unelegant.)

               Lready = 0
               Do while(Lready.eq.0)
                 Itemp = 1 + Int(Dble(Ntemp)*Ran_Uniform())
                 Jtemp = 1 + Itemp
                 If(Jtemp.Le.Ntemp) Then
                   Lready = 1
                 Endif
               Enddo
            
               Swapb(Itemp) = Swapb(Itemp) + 1.0d0

Ccccccccccccccccccccccccccccccccccccccccccccccccc
C     Check For Acceptance: Swap The Systems    C
Ccccccccccccccccccccccccccccccccccccccccccccccccc

               If(Ran_Uniform().Lt.Dexp((Beta(Itemp)-Beta(Jtemp))*
     &              (Uold(Itemp)-Uold(Jtemp)))) Then

                  U            = Uold(Itemp)
                  Uold(Itemp)  = Uold(Jtemp)
                  Uold(Jtemp)  = U
                  Swapa(Itemp) = Swapa(Itemp) + 1.0d0

                  Do J=1,NUMBEROFPARTICLES
                     U           = Rx(J,Itemp)
                     Rx(J,Itemp) = Rx(J,Jtemp)
                     Rx(J,Jtemp) = U

                     U           = Ry(J,Itemp)
                     Ry(J,Itemp) = Ry(J,Jtemp)
                     Ry(J,Jtemp) = U
                  Enddo
               Endif
