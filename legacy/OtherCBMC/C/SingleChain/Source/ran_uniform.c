...
//       Random number from a uniform distribution on a sphere: 
//       returns the X, Y and Z coordinates of a point on a sphere of radius 1 and center (0,0,0)
//       This subroutine follows the algorithm proposed by Marsaglia (1972):
...
void RanSphere(VECTOR *vec)
{
  ...
  do
  {
    Ran1=2.0*RandomNumber()-1.0; // RandomNumber() is a call to a function that returns a random number from a uniform distribution in the interval [0,1]
    Ran2=2.0*RandomNumber()-1.0;
    Ransq=Ran1*Ran1+Ran2*Ran2;
  } while(Ransq>=1.0);
