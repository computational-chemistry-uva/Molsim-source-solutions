...
void MonteCarlo::randomSwap()
{
//     if (uniform() < 0.5)
//     {
//         double3 newParticle = boxSize * double3(uniform(), uniform(), uniform());
//         EnergyVirial newEnergyVirial = particleEnergyVirial(positions, newParticle, numberOfParticles + 1, boxSize,
//         cutOff, sigma, epsilon);

//         double tailEnergyDifference = (2 * numberOfParticles + 1) * cutOffPrefactor.energy / volume;
//         newEnergyVirial.energy += tailEnergyDifference;

//       double tailVirialDifference = (2 * numberOfParticles + 1) * cutOffPrefactor.virial / (volume * volume);
//       newEnergyVirial.virial += tailVirialDifference;

//         if (uniform() < (pressure * volume / (numberOfParticles + 1)) * std::exp(-beta * newEnergyVirial.energy))
//         {
//           positions.push_back(newParticle);
//           numberOfParticles++;
//           density = numberOfParticles / volume;
//           // runningEnergyVirial = systemEnergyVirial(positions, boxSize, cutOff, sigma, epsilon, cutOffPrefactor);
//           runningEnergyVirial += newEnergyVirial;
//         }
//     }
//     else
//     {
//       size_t particleIdx = static_cast<size_t>(uniform() * numberOfParticles);
//       EnergyVirial newEnergyVirial = particleEnergyVirial(positions, positions[particleIdx], particleIdx, boxSize,
//       cutOff, sigma, epsilon);
...
//       if (uniform() < (density / pressure) * std::exp(beta * newEnergyVirial.energy))
//       {
//           positions.erase(positions.begin() + particleIdx);
//           numberOfParticles--;
//           density = numberOfParticles / volume;
//           // runningEnergyVirial = systemEnergyVirial(positions, boxSize, cutOff, sigma, epsilon, cutOffPrefactor);
//           runningEnergyVirial -= newEnergyVirial;
